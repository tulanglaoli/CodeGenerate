...
namespace DotNet.Utilities
{
    public class XMLProcess
    {
        ...
        public XMLProcess(string strPath)
        {
            ...
            _Xmldoc = XMLLoad(this._XMLPath);
        }

        private static XMLProcess instance = null;

        /// <summary>
        /// 单例（新加2.0方法）
        /// </summary>
	    public static XMLProcess Instance
	    {
		    get
		    {
			    if (instance == null)
			    {
				    instance = new XMLProcess();
        ...
        private XmlDocument XMLLoad()
        {
            ...
            {
                AppExceptionLog.Instance.WriteExceptionLog("XMLLoad", e);
            }
            return xmldoc;
        }
        ...
        private static XmlDocument XMLLoad(string strPath)
        {
            ...
            try
            {
                ...
                string filename = GetXmlFullPath( strPath);
                if (File.Exists(filename)) xmldoc.Load(filename);
            }
            catch (Exception e)
            {
